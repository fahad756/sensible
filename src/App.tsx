...
export default function App() {
  ...
  useEffect(() => {
    if (visionEnabled && scanningActive) {
      startVisionLoop();
    } else {
      stopVisionLoop();
    }
  ...
  }, [visionEnabled, scanningActive]);

  useEffect(() => {
    if (audioAssistEnabled && scanningActive) {
      startAudioAssist();
    } else {
      stopAudioAssist();
    }
  ...
  }, [audioAssistEnabled, scanningActive]);
          ...
          <section className="glass rounded-lg p-4">
            ...
            <div className="mt-5 space-y-4">
              ...
              {isCapturing && (
                <button
                  type="button"
                  onClick={() => setScanningActive((prev) => !prev)}
                  className={`inline-flex w-full min-h-12 items-center justify-center gap-2 rounded-md px-4 py-3 font-semibold transition ${
                    scanningActive
                      ? "bg-amber-400 text-slate-950 hover:bg-amber-300"
                      : "bg-slate-100 text-slate-950 hover:bg-white"
                  }`}
                >
                  {scanningActive ? <Pause size={18} /> : <Play size={18} />}
                  {scanningActive ? "Pause Scanning" : "Start Scanning"}
                </button>
              )}
            </div>
          </section>
